...
void *aligned_malloc(size_t size, size_t alignment) {
    ...
    m_ptr = (uintptr_t)malloc(size + (2*alignment));

    ptr = (m_ptr & (~mask)) + (2*alignment); // return ptr

    // store m_ptr
    *((uintptr_t *)(ptr - sizeof(uintptr_t))) = m_ptr;
...
void aligned_free(void *ptr) {
    ...
    m_ptr = *((uintptr_t *)((uintptr_t)ptr - sizeof(uintptr_t)));
